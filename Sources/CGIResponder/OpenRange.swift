 ...
 */
public struct OpenRange<Bound: Comparable>: RangeExpression {
...
infix operator <-<: RangeFormationPrecedence
public func <•< <T>(lhs:T, rhs:T) -> OpenRange<T> where T:Comparable {
  guard lhs < rhs else { fatalError("OpenRange: `lower` must be less than `upper`") }
  return OpenRange<T>(uncheckedBounds:(lower:lhs, upper:rhs))
}
public func <-< <T>(lhs:T, rhs:T) -> OpenRange<T> where T:Comparable {
  return lhs<•<rhs
}
...
extension OpenRange {
  public func overlaps(_ other:ClosedRange<Bound>) -> Bool {
    return (!other.isEmpty && self.contains(other.upperBound)) ||
      (!self.isEmpty && other.contains(self.upperBound) && self.upperBound != other.lowerBound)
  }
