...
extension Array where Element == CacheControlDirective {
  ...
  internal init?(string:String) {
    let directiveStrings = string.components(separatedBy:",")
    
    self.init()
    for ss in directiveStrings {
      guard let directive = CacheControlDirective(rawValue:ss.trimmingUnicodeScalars(in:.whitespaces)) else {
        return nil
      }
      self.append(directive)
    }
  }
  internal init?(_ value:HTTPHeaderFieldValue) {
    self.init(string:value.rawValue)
  }
  
  // remove overlapping
  internal func arranged() -> [CacheControlDirective] {
