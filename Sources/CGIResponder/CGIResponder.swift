...
extension CGIResponder {
  ...
  /// If any error is thrown, the response will be generated by `fallback`.
  ///
  /// # Sample Code
  /// ```Swift
  ...
  ///  responder.respond(to: &output, fallback: Fallback.self)
  ///
  ///  /*
  ///   -- OUTPUT --
    ...
    do {
      ...
      let cgiError: CGIError = ({
        if case let error as CGIError = error {
          ...
          return _VersatileCGIError(localizedDescription: error.errorDescription ?? error.localizedDescription)
        } else {
          let error = error as NSError
          return _VersatileCGIError(localizedDescription: error.localizedFailureReason ?? error.localizedDescription)
        }
      })()
      let fallback = fallbackType.init(error: cgiError)
      let fallbackResponder = CGIResponder(status: fallback.status, content: fallback.fallbackContent)
      try! fallbackResponder.respond(to: &output) { _ in true }
    }
  }
}
