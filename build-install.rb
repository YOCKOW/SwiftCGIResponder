...
ModuleVersion = "0.1.0-dev20171204.v2.next"
...
if !Options[:skip_build]
  puts("[Start Building...]")
  try_exec("#{Options[:swift].escaped()} build #{swiftBuildArguments}", 2)
  puts()
end
...
if !Options[:skip_test]
  ...
  if !Options[:debug]
    ...
    try_exec("#{Options[:swift].escaped()} test --build-path #{Options[:build_directory].escaped()} --configuration #{configuration}", 2)
  end
  puts()
end
